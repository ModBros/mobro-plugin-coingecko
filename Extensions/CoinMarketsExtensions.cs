...
public static class CoinMarketsExtensions
{
  extension(CoinMarkets cm)
  {
    ...
    public IEnumerable<MetricValue> MapToMetricValues()
    {
      ...
      yield return new MetricValue(MetricId(cm.Id, "price_change_percentage_24h"), cm.GetLastUpdated(),
        cm.PriceChangePercentage24H);
      ...
      yield return new MetricValue(MetricId(cm.Id, "ath_change_percentage"), cm.GetLastUpdated(),
        cm.AthChangePercentage);
      yield return new MetricValue(MetricId(cm.Id, "ath_date"), cm.GetLastUpdated(), cm.AthDate);
    }

    private DateTime GetLastUpdated() =>
      cm.LastUpdated?.UtcDateTime ?? DateTime.UtcNow;
  }
